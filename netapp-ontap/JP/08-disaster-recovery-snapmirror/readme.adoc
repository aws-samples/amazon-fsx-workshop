...
. *Amazon FSx for NetApp ONTAP ワークショップの Linux インスタンス* の SSH 接続に *_戻り_* 、*セカンダリー* ファイルシステムのためのファイルシステム管理エンドポイント IP アドレスをエクスポートします。以下に示した IP アドレスを、その *ファイルシステムの管理エンドポイント - IP アドレス* に置き換えます。
+
...
----
+
. *Amazon FSx for NetApp ONTAP ワークショップの Linux インスタンス* の SSH 接続に *_戻り_*、Linux EC2 インスタンスにボリュームを *マウント* します。以下に示す IP アドレスを SVM の NFS エンドポイントの IP アドレスに置き換えます。
+
[source,bash]
----
SMMOUNT="/snapmirror"
...
+
. *Amazon FSx for NetApp ONTAP ワークショップの Linux インスタンス* の SSH 接続に *戻り*、*セカンダリー* ファイルシステム管理エンドポイントに SSH で接続します。
+
[source,bash]
----
ssh ${ADMINUSER}@${DRMGMTENDPOINT}
