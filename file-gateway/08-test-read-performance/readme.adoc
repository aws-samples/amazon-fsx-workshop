...
+
. Copy the following table to your local computer and record the results in the (Test 1) column.
. Repeat the test and record the results in the (Test 2) column.
+
[width="50%",cols="d,d,d,d,d",frame="topbot",options="header"]
|===
h| EC2 Instance (us-west-1) 2+h| FSx for Windows File Server (us-east-1) 2+h| FSx File Gateway (us-west-1)
